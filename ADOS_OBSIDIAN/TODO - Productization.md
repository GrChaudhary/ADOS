# TODO — Productization ("this is a product now, not a demo")

Created: 2026-08-07
Companion to [[TODO - Orchestration Platform Pivot]] (feature/architecture work)
and `../orchestration-platform-vision.md` (the product vision).

**This file tracks a different axis than the pivot TODO.** That one asks "what
can the platform *do*?" — and the answer is genuinely a lot. This one asks
"can anyone other than one developer on one laptop actually *run* it?" — and
today the answer is no. Every item below was verified against the actual code
on 2026-08-07, not taken from any existing doc.
...
## Stage 1 — Stop the bleeding (do this week, before anything else)

[In plain terms: four things that are cheap to do, and until they're done, everything else is built on sand.]

- [ ] **Commit and push. Today.** Last commit is `0c6927c`, dated
...
- [ ] **Dockerfile + a real `docker compose up`.** There is currently **no
...
- [ ] **CI (GitHub Actions): lint + typecheck + `pytest` against service
...
- [ ] **Make the demo seed opt-in.** `backend/app/main.py` unconditionally
  ...
  startup. Gate it behind `ADOS_SEED_DEMO_DATA` (default off).
