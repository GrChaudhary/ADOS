...
def test_knowledge_graph_get_specification(kg):
    spec = kg.getSpecification("P-1002")
    assert spec is not None
    assert spec.spec_id == "SP-1002"
    assert spec.nominal == 45.0
    assert spec.tolerance_plus == 0.05


def test_knowledge_graph_find_affected_products(kg):
    products = kg.findAffectedProducts("SP-1002")
    assert len(products) > 0
    assert any(p.sku == "PROD-500" for p in products)


def test_knowledge_graph_find_approved_substitutes(kg):
    substitutes = kg.findApprovedSubstitutes("P-1002")
    assert len(substitutes) > 0
    assert any(p.part_number == "P-1002B" for p in substitutes)
...
def test_causal_isolation_agent_end_to_end(context, kg, cg):
    ...
    assert output.result["primary_root_cause"] == "Tolerance drift on Line 3 CNC Spindle"
    assert "PROD-500" in output.result["affected_products"]
...
def test_cad_spec_agent(context, kg):
    ...
    stage_in = StageInput(stage_name="Reasoning", payload={"part_number": "P-1002", "measured_value": 45.08})
    output, envelope = agent.run(context, stage_in)
...
def test_substitution_agent(context, kg):
    ...
    stage_in = StageInput(stage_name="CandidateGeneration", payload={"part_number": "P-1002"})
    output, envelope = agent.run(context, stage_in)

    assert output.result["has_approved_substitute"] is True
    assert output.result["top_candidate"]["target_part_number"] == "P-1002B"
