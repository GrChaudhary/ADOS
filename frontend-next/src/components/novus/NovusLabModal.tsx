...
interface NovusLabModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function NovusLabModal({ isOpen, onClose }: NovusLabModalProps) {
  const [activeTab, setActiveTab] = useState<"agents" | "telemetry" | "logs">("agents");
  const [promptInput, setPromptInput] = useState("");
  const [simulatedLogs, setSimulatedLogs] = useState<string[]>([
    "[NOVUS_INIT] NOVUS ADOS Autonomous Decision OS Engine active.",
    "[CAUSAL_GRAPH] 4,280 incident precedents indexed in IBM Cloudant NoSQL.",
    "[WATSONX_ITSM] IBM Cloud IAM bearer token active • Latency 42ms.",
    "[DIGITAL_TWIN] CNC Spindle vibration telemetry streaming (0.18 mm/s).",
    "[GOVERNANCE] Tier-0 autonomous execution policy active for confidence > 95%.",
  ]);
  ...
  const handleRunCommand = (e: React.FormEvent) => {
    ...
    const newLog = `[OPERATOR_PROMPT] ${promptInput} -> Invoking Causal Isolation & Impact Simulation Agents...`;
    const responseLog = `[NOVUS_AI] Recommendation: Execute Tier-0 autonomous parameter shift (+0.05mm tolerance) & soft-reserve SAP inventory. Confidence 98.4%. MTTR reduced by 42 mins.`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-3xl jarvis-glass-card border border-purple-500/30 bg-[#0c0824]/95 text-white shadow-2xl flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-6 border-b border-purple-500/20 bg-purple-950/30">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-cyan-500 via-purple-600 to-pink-500 flex items-center justify-center font-orbitron font-bold text-white shadow-lg shadow-purple-500/30">
              N
            </div>
            <div>
              <h2 className="text-xl font-orbitron font-bold jarvis-gradient-text tracking-wide">
                NOVUS ADOS SECRET LAB
              </h2>
              <p className="text-xs text-purple-300/70 font-mono">
                L6 Enterprise Intelligence & 8 Specialist AI Agents Suite
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 p-8 overflow-y-auto space-y-6">
          {activeTab === "agents" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                {
                  name: "ParameterAdjustmentAgent",
                  desc: "Tunes machine CNC spindle parameters to prevent drift.",
                  status: "ACTIVE • TIER 0",
                  color: "from-amber-500/20 to-orange-500/20 border-amber-500/30",
                },
                {
                  name: "FeedbackCalibrationAgent",
                  desc: "Recalibrates Bayesian causal graph weights after incident recovery.",
                  status: "ACTIVE • LEARNING",
                  color: "from-indigo-500/20 to-pink-500/20 border-indigo-500/30",
                },
              ].map((agent, idx) => (
                <div
                  key={idx}
                ...
                >
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-purple-500/20 text-pink-300 border border-purple-500/40">
                        {agent.status}
                      </span>
                    </div>
          ...
          {activeTab === "telemetry" && (
            <div className="space-y-4">
              ...
              <div className="p-6 rounded-2xl bg-purple-950/40 border border-purple-500/30 flex justify-between items-center">
                <div>
                  <h4 className="text-sm font-orbitron font-bold text-white mb-1">
                    IBM Cloudant RAG Decision Memory
                  </h4>
                  <p className="text-xs text-purple-300/70 font-mono">4,280 Precedent Vectors Indexed</p>
                </div>
                <div className="text-right">
                  <span className="text-xl font-orbitron text-purple-300 font-bold">18 ms</span>
                  <p className="text-[10px] text-emerald-400 font-mono">READY</p>
                </div>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleRunCommand} className="flex gap-3">
            <input
              ...
              placeholder="Ask NOVUS ADOS Copilot (e.g. 'Optimize MTTR for Spindle Drift on Line 2')..."
