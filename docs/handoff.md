...
- **Phase 0 (dataset)**: originally a "Nova Motors Detroit Plant" dataset; **superseded by the Plant 04 Bangalore, Karnataka rename** — see §6.
...
2. **Enterprise Asset Model (EAM)** (`knowledge/asset_model.py`): Ground truth hierarchy (`Plant > Factory > Line > Machine > PLC > Sensor > Product > Component`), separating operational physical topology from Knowledge Graph reasoning. `KnowledgeGraph.resolveAssetLineage()` delegates to it. Still single-plant (`PLANT-04-BANGALORE > FAC-P04`, 4 lines) — no multi-plant data yet.
