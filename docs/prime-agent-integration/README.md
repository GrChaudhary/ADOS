...
| [22-metrics-alerting-operationalization.md](22-metrics-alerting-operationalization.md) | P11 follow-up — wires the metrics export surface into a real, local Prometheus + Alertmanager and proves the chain end to end |
| [23-p12-production-operationalization.md](23-p12-production-operationalization.md) | P12 — distributed admission control, a closed NULL-expiry gap, automatic capability-request reconciliation, rate limiting, and the Model B verdict |
| [24-p13-horizontal-scale-out.md](24-p13-horizontal-scale-out.md) | P13 — the real `--workers 2+` blocker (the manufacturing-incident approval queue, not the Dockerfile's stated reason), a closed admission-control bypass, and the Model C scale-out verdict |
| [25-p14-capability-registry-consistency.md](25-p14-capability-registry-consistency.md) | P14 — closes P13's two deferred gaps (hot-disable cache staleness, dynamic-capability dispatch propagation), proven live across real, separate OS processes |
| [26-p15-concurrency-atomicity-review.md](26-p15-concurrency-atomicity-review.md) | P15 — full concurrency/atomicity audit (approval, idempotency, reconciliation, admission, token expiry); finds and fixes two real defects (a late-completion race, an admission-control leak), proven live including a real SIGKILL crash boundary |
| [27-multi-tenancy-and-multi-host-safety.md](27-multi-tenancy-and-multi-host-safety.md) | P16 — confirms live, for the first time, that no tenant isolation model exists anywhere in ADOS; finds and fixes a real multi-host Docker orphan-sweep misattribution defect |
| [28-multi-tenancy-and-tenant-isolation.md](28-multi-tenancy-and-tenant-isolation.md) | P17 — builds a real tenant model (native SQLAlchemy `do_orm_execute` filter, fail-closed by default) and closes the exact cross-tenant defect P16 proved live, with two real tenants and two real users |
| [29-p18-tenant-production-hardening.md](29-p18-tenant-production-hardening.md) | P18 — finds and closes a real mission-creation tenant-misattribution gap P16/P17 both missed, re-evaluates Postgres RLS with a sharper concrete blocker, and adds a genuine multi-process/multi-tenant proof (real OS processes, real concurrent asyncio, an adversarial background-job context override) — the full Model A/B/C readiness re-audit |
| [30-post-p18-stabilization-and-model-c-closure-plan.md](30-post-p18-stabilization-and-model-c-closure-plan.md) | Post-P18 audit-and-plan (no code changes): confirms P11-P18 preserved and protected paths untouched, finds zero merge candidates across 22 sibling repos, and traces each remaining Model C blocker to exact source locations with a minimal, non-speculative closure design for each — implementation deliberately deferred |
| [31-model-c-decision-gate.md](31-model-c-decision-gate.md) | Model-C decision gate (no code changes): classifies each of the four remaining blockers as engineering / product / architecture / environment, finding only permanent dead-host Docker reclamation is a genuine required build — RLS stays deferred as defense-in-depth, tenant capacity is a product question, two-host proof is an environmental gap not a design gap |
| [32-dead-host-reclamation-and-frontend-development.md](32-dead-host-reclamation-and-frontend-development.md) | Closes doc 31's one engineering blocker (heartbeat-based dead-host Docker/workspace reclamation, 14 new tests, zero regressions) and brings `frontend-next/` back into active development: closes a CRITICAL tenant-header gap and builds the previously-nonexistent runtime capability-request approval queue UI |
